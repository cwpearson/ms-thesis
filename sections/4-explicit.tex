...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
    \Statex
    \Function{cpu\_read}{$ptr$, $count$, $stride$}
        \State \Return 
    \EndFunction
    \Statex
...
The peak achieved 20 GB/s transfer bandwidth on S822LC is 55\% of the theoretical 40 GB/s transfer provided by the NVLink 1.0 interconnect between CPU0 and GPU0 (Section~\ref{sec:minsky}).
Similarly, on AC922, the 30 GB/s achieved bandwidth is substantially lower than the 75 GB/s advertised by the NVLink 2.0.
In these cases, there is an internal application-to-driver memory copy that is limiting the overall transfer performance.
...
Figures~\ref{fig:minsky_pageable_affinity} and \ref{fig:hal_pageable_affinity} show that affinity has a negligable effect on transfer performance for pagable transfers.
These transfers are likely limited by the speed of the system memory copy from pageable to pinned buffer.
...
\begin{table}[ht]
    ...
    Pageable $\rightarrow$ GPU & $\times$ (Fig.~\ref{fig:minsky_pageable_affinity}) & $\times$   (Fig.~\ref{fig:hal_pageable_affinity}) & \\ \hline
    Pageable $\leftarrow$ GPU  & for intermediate sizes (Fig.~\ref{fig:minsky_pageable_affinity}) & for intermediate sizes (Fig.~\ref{fig:hal_pageable_affinity}) & \\ \hline
...
\begin{table}[ht]
    ...
    Pageable $\leftarrow$ GPU  & $\times$ & $\times$ & \\ \hline

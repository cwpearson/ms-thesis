...
\begin{lstlisting}[language=C++, caption=CPU-CPU Read Function, label=lst:explicit-cpu-cpu-cpuread, numbers=left]
void rd_8(char *ptr, 
          const size_t count, 
          const size_t stride) {
...
\begin{lstlisting}[language=C++, caption=CPU-CPU Write Function, label=lst:explicit-cpu-cpu-cpuwrite, numbers=left]
void wr_8(char *ptr, 
          const size_t count, 
          const size_t stride) {
  int64_t *dp = reinterpret_cast<int64_t*>(ptr);
...
For both the \texttt{rd\_8} and \texttt{wr\_8} operations, \texttt{ptr} is the allocation that is being read or written,
...
Since memory loads in isolation are side-effect free, \texttt{DoNotOptimize} and \texttt{ClobberMemory} are used to prevent loads from being optimized away by the compiler.
...
\begin{algorithm}
	\caption[test]{
		...
		\texttt{rd\_8} is defined in Listing~\ref{lst:explicit-cpu-cpu-cpuread} and \texttt{wr\_8} is defined in Listing~\ref{lst:explicit-cpu-cpu-cpuwrite}.
	}
...
\begin{figure}[ht]
	...
	\begin{subfigure}[b]{0.31\textwidth}
		\includegraphics[width=\textwidth]{figures/generated/s822lc_cpu0-cpu0.pdf}
		\caption{}
		\label{fig:explicit-s822lc-cpu0-cpu0}
	\end{subfigure}
	~
	\begin{subfigure}[b]{0.31\textwidth}
		\includegraphics[width=\textwidth]{figures/generated/ac922_cpu0-cpu0.pdf}
		\caption{}
		\label{fig:explicit-ac922-cpu0-cpu0}
	\end{subfigure}
	~
	% \begin{subfigure}[b]{0.31\textwidth}
	...
	\caption[Intra-CPU \texttt{cudaMemcpy} Bandwidth]{
        \texttt{cudaMemcpy} CPU0-to-CPU0 bandwidth measured through several means.
		1T RD and 8T RD refer to one-thread and eight-thread methods described in Algorithm~\ref{alg:explicit-cpu-cpu}.
		cudaMemcpy refers to a \texttt{cudaMemcpy} between a pageable allocation and pinned allocation on CPU0.
		(a) shows results for S822LC.
		(b) shows results for AC922.
		\todo{(c) shows results for DGX-1}
    }
	\label{fig:explicit-cpu0-cpu0}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		\includegraphics[width=\textwidth]{figures/generated/s822lc_rd_cpu.pdf}
		\caption{}
		\label{fig:s822lc-cpu0-cpu1-dst}
	\end{subfigure}
	~
	\begin{subfigure}[b]{0.45\textwidth}
		\includegraphics[width=\textwidth]{figures/generated/s822lc_wr_cpu.pdf}
		\caption{}
		\label{fig:s822lc-cpu0-cpu1-src}
	\end{subfigure}
	\\
	\begin{subfigure}[b]{0.45\textwidth}
		\includegraphics[width=\textwidth]{figures/generated/ac922_rd_cpu.pdf}
		\caption{}
		\label{fig:ac922-cpu0-cpu1-dst}
	\end{subfigure}
	~
	\begin{subfigure}[b]{0.45\textwidth}
		\includegraphics[width=\textwidth]{figures/generated/ac922_wr_cpu.pdf}
		\caption{}
	...
	\caption[CPU0 to CPU1 transfer bandwidth]{
		...
		Whiskers at each point show the standard deviation measured over 10 repetitions.
	}
	\label{fig:cpu0-cpu1}
\end{figure}
...
Figure~\ref{fig:explicit-cpu0-cpu0} shows the performance of using \texttt{cudaMemcpy} to only do a copy from a pageable allocation to a pinned allocation.

On AC922, the pageable-to-GPU transfer for large transfer sizes is actually faster than the pageable-to-pinned transfer that is should be limited by.
This suggests that there is a different implementation for the two cases.
Bandiwdth spikes at intermediate sizes suggest that the GPU DMA may directly access data from the CPU cache when the transfer can fit in the cache.

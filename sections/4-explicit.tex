...
Algorithm~\ref{alg:explicit-gpu-gpu-peer} is used to evaluate the achievable GPU-GPU transfer bandwidth with peer access enabled.
First, peer access is enabled.
If the devices do not support peer access, this will fail, and the benchmark is not run.
...
Algorithm~\ref{alg:explicit-gpu-gpu-nopeer} is used to evaluate GPU-GPU transfer bandwidth without peer access.
First, peer access is disabled.
Then, to help ensure consistency in the benchmarks, the ecexuting thread is pinned to a specific NUMA node.
On S822LC, this was observed to have a large impact on GPU-GPU transfer performance.
When execution is pinned to the NUMA node affiliated with the source GPU, transfer bandwidth is more than $25\%$ higher.
No similar pattern was observed on AC922.

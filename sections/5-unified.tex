...
\begin{algorithm}[H]
	\caption[Measuring GPU-GPU unified memory coherence or prefetch bandwidth.]{
		Measuring GPU-GPU unified memory coherence or prefetch bandwidth.
...
\begin{algorithm}[H]
	\caption[Measuring CPU-GPU Unified Memory Coherence or Prefetch Bandwidth.]{
		Measuring CPU-GPU Unified Memory Coherence or Prefetch Bandwidth.
		\texttt{gpu\_write} is defined in Listing~\ref{lst:gpu-write}.
		\todo{describe arguments}
	}
...
\begin{algorithm}[H]
	\caption[Measuring GPU-to-CPU unified memory coherence bandwidth.]{
		Measuring GPU-to-CPU unified memory coherence bandwidth.
...
\begin{algorithm}[H]
	\caption[Measuring GPU-to-CPU unified memory prefetch bandwidth.]{
		Measuring GPU-to-CPU unified memory prefetch bandwidth.
		\texttt{cpu\_write} is defined in Listing~\ref{lst:cpu-write}.
		\todo{describe arguments}
	}
...
\todo{describe cpu\_write and gpu\_write}.

The \texttt{gpu\_write} workload, shown in Listing~\ref{lst:cpu-write} and \ref{lst:gpu-write}, writes a single value to every $stride$ bytes in $bytes$.
...
Algorithm~\ref{alg:um-coherence-bw} describes a similar approach for measure prefetch bandwidth between two devices participating in the unified memory system.

...
\begin{algorithm}[H]
	\caption[Measuring GPU-GPU Unified Memory Coherence Bandwidth.]{
		Measuring GPU-GPU Unified Memory Coherence Bandwidth.
		\texttt{gpu\_write} is defined in Listing~\ref{lst:gpu-write}
	}
	\label{alg:um-coherence-bw-gpu-gpu}
	\begin{algorithmic}[1]
		\Statex
		...
		\State \texttt{memset($ptr$, 0, $transfer\_size$)} \Comment force pages to be allocated

		\State cudaEventCreate($start$)
		\State cudaEventCreate($stop$)		        
				
		\For{$state$}
			\State cudaMemPrefetchAsync($ptr$, $transfer\_size$, $src$)
			\State cudaSetDevice($src$)
			\State cudaDeviceSynchronize()
			\State cudaSetDevice($dst$)
			\State cudaDeviceSynchronize()
			\State cudaEventRecord($start$)
			\State \texttt{gpu\_write<<<$256$,$256$>>>($ptr$, $transfer\_size$, $stride$)}
